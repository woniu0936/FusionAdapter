package com.fusion.example.core.repo

import com.fusion.example.core.model.*
import kotlinx.coroutines.delay
import kotlin.random.Random

object MockSource {
    private val elena = User("1", "Elena Vance", "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150")
    private val gordon = User("2", "Gordon Freeman", "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150")

    suspend fun getMoments(): List<Moment> {
        delay(800)
        return List(15) { i ->
            Moment("m_$i", if (i % 2 == 0) elena else gordon, 
                "Discovering the elegance of FusionAdapter. M3 + UDF + Fusion = Pure Magic.",
                if (i % 3 == 0) listOf("https://picsum.photos/seed/$i/800/600") else emptyList(),
                Random.nextInt(10, 500))
        }
    }

    suspend fun getMarket(): List<Any> {
        delay(800)
        return mutableListOf<Any>().apply {
            add(SectionHeader("Hot Deals"))
            addAll(List(4) { i -> Product("hp_$i", "Premium Product $i", "$1,999", "https://picsum.photos/seed/${i+10}/500/500", true) })
            add(SectionHeader("All Collections"))
            addAll(List(20) { i -> Product("p_$i", "Standard Item $i", "$99", "https://picsum.photos/seed/${i+100}/500/500", false, Random.nextInt(400, 700)) })
        }
    }

    suspend fun getChat(): List<ChatMessage> {
        delay(500)
        return List(40) { i ->
            val isMe = i % 2 == 0
            ChatMessage("c_$i", 
                if (i % 5 == 0) "System maintenance scheduled for 2:00 AM." else "Sample message #$i from Fusion Messenger.", 
                if (i % 5 == 0) 3 else 1, 
                isMe, 
                if (!isMe) elena else null)
        }
    }
}