...
class KtxManualActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fusionAdapter.register<TextItem, ItemTextBinding>(ItemTextBinding::inflate) {
            onBind { item ->
        ...
        // 2. 注册另一种类型
        fusionAdapter.register<ImageItem, ItemImageBinding>(ItemImageBinding::inflate) {
            onBind { item ->
        ...
        // 一句话开启自动滚动
        binding.recyclerView.autoScrollToBottom(fusionAdapter)
        binding.fabAdd.setOnClickListener {
            // KTX 也可以给手动挡 Adapter 扩展类似 += 的操作符 (需自行在 KTX 定义或直接调用 addItems)
            fusionAdapter.addItems(listOf(TextItem("3", "New DSL Item")))
        }
    }
}