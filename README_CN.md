...
FusionAdapter 内置了强大的**诊断系统**，帮助你检查注册状态并识别性能瓶颈（例如 `onCreateViewHolder` 耗时过长）。

### 1. 开启调试模式
确保在配置中启用了调试模式（指标仅在调试模式下收集，以避免 Release 包的性能损耗）。
...
### 2. 输出报告 (Dump)
在任何 Fusion Adapter 实例上调用 `dump()` 即可将格式化的报告输出到 Logcat。
